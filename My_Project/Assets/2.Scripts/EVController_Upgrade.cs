...
public class EVController_Upgrade : MonoBehaviour
{
    ...
    public float sceneBaselineEV = 13f; // 예: 맑은 날 EV 15 (Sunny 16 rule)
    ...
    private void UpdateCameraEffects()
    {
        ...
        int apertureIndex = (apertureSlider != null) ? (int)apertureSlider.value : 3; // 기본값 f/4.0 (인덱스 3)
        int shutterIndex = (shutterSlider != null) ? (int)shutterSlider.value : 3; // 기본값 1/125s (인덱스 3)
        int isoIndex = (isoSlider != null) ? (int)isoSlider.value : 2;     // 기본값 400 (인덱스 2)
